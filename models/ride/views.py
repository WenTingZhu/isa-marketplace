...
def ride(request, id):
    ...
    if request.method == 'GET':
        try:
            ...
            data = {'ride_status': str(ride.status), 'dropOffLocations': str(dropoffLocations), 'passengers': str(passengers), 'departure': str(
                "{:%b %d, %Y %H:%M}".format(ride.departure)), 'available_seats': str(ride.openSeats), 'driver': str(driver), 'status': str(HTTP_200_OK)}
...
if request.method == 'GET':
    try:
        ...
        data = {'No rides found.', 'status': str(HTTP_404_NOT_FOUND)}
        return JsonResponse(data, status=HTTP_404_NOT_FOUND)
